...
const NavBar: React.FC = () => {
  ...
  const navLinksTranslated = navLinks.map(link => {
    if (link.label === 'Home') return { ...link, label: t('home') };
    if (link.label === 'Contact Us') return { ...link, label: t('contact') };
    if (link.label === 'Donate') return { ...link, label: t('donate') };
    return link;
  });
  ...
  return (
    <AppBar position="static" color="default" elevation={2} style={{ background: 'rgba(255,255,255,0.85)', backdropFilter: 'blur(8px)', borderRadius: 16, margin: '1rem auto', maxWidth: 1200, boxShadow: '0 4px 16px rgba(44,32,18,0.10)' }}>
      <Toolbar variant={isMobile ? 'dense' : 'regular'} style={{ justifyContent: 'center', flexWrap: 'wrap', borderRadius: 16 }}>
        ...
        {/* Right: Theme and Language Switchers */}
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, ml: 2 }}>
          <IconButton onClick={colorMode.toggleColorMode} color="inherit" aria-label="Toggle dark/light mode">
            {theme.palette.mode === 'dark' ? <Brightness7Icon /> : <Brightness4Icon />}
          </IconButton>
          <Box sx={{ display: 'flex', alignItems: 'center', ml: 1, borderRadius: 2, bgcolor: 'rgba(191,161,74,0.08)', px: 1, py: 0.5 }}>
            <TranslateIcon sx={{ color: '#bfa14a', mr: 0.5 }} />
            <Button
              onClick={() => setLang('en')}
              sx={{
                minWidth: 32,
                color: lang === 'en' ? '#bfa14a' : theme.palette.text.primary,
                fontWeight: lang === 'en' ? 700 : 500,
                fontSize: '1rem',
                px: 0.5,
                textTransform: 'none',
                '&:hover': { bgcolor: 'transparent', color: '#bfa14a' },
              }}
              disableRipple
            >
              EN
            </Button>
            <Typography variant="body2" sx={{ mx: 0.5, color: '#bfa14a', fontWeight: 700 }}>|</Typography>
            <Button
              onClick={() => setLang('sh')}
              sx={{
                minWidth: 32,
                color: lang === 'sh' ? '#bfa14a' : theme.palette.text.primary,
                fontWeight: lang === 'sh' ? 700 : 500,
                fontSize: '1rem',
                px: 0.5,
                textTransform: 'none',
                '&:hover': { bgcolor: 'transparent', color: '#bfa14a' },
